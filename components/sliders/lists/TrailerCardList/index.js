...
const TrailerCardList = ({ items = junkData, maxElementCount = 4, ...props }) => {
    ...
    responsive: [
      {
        breakpoint: 1536,
        settings: {
          slidesToShow: Math.max(maxElementCount - 1, 1)
        }
      },
      {
        breakpoint: 1280,
        settings: {
          slidesToShow: Math.max(maxElementCount - 2, 1)
        }
      },
      {
        breakpoint: 768,
        settings: {
          slidesToShow: Math.max(maxElementCount - 3, 1)
        }
      }
    ]
  };
  ...
  return (
    <S.StyledContainer>
      ...
      <Slider {...settings}>
        {items.map((el, idx) => (
          <TrailerCard key={idx} index={idx} item={el} />
        ))}
      </Slider>
    </S.StyledContainer>
  );
